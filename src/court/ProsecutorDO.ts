...
/**
 * Prosecutor Durable Object — one instance shared across cases.
 * Receives case context from the debate workflow, generates adversarial arguments
 * against the proposed decision, and stores them in local SQLite for cross-session memory.
 */
export class ProsecutorDO extends DurableObject<Env> {
  ...
  private async handleArgue(request: Request): Promise<Response> {
    const { caseId, caseCtx, rebuttalTargetId } = (await request.json()) as {
      caseId: string;
      caseCtx: CaseContext;
      rebuttalTargetId?: string;
    };

    const text = await generateArgument(this.env.AI, "prosecutor", caseCtx);

    const id = crypto.randomUUID();
    const now = new Date().toISOString();

    this.ctx.storage.sql.exec(
      `INSERT INTO agent_arguments (id, case_id, round, text, rebuttal_target_id, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
      id,
      caseId,
      caseCtx.round,
      text,
      rebuttalTargetId ?? null,
      now,
    );
