...
/**
 * Defender Durable Object — one instance shared across cases.
 * Receives case context from the debate workflow, generates steelman arguments
 * in favour of the proposed decision, and stores them in local SQLite for cross-session memory.
 */
export class DefenderDO extends DurableObject<Env> {
  ...
  private async handleArgue(request: Request): Promise<Response> {
    ...
    const text = await generateArgument(this.env.AI, "defender", caseCtx);
